...
def calc_score(ingredients, ingredients_calories, amonts):
    if sum(cal*a for cal, a in zip(ingredients_calories, amonts)) != 500:
        return 0
...
def main():
    try:
        ...
        while True:
            ...
            ingredients_calories += [calories]
    except EOFError:
        limit = 100
        print(max(calc_score(ingredients, ingredients_calories, amonts) for amonts in iter_sumto(limit, len(ingredients))))
